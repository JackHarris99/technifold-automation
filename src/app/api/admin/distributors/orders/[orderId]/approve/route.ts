/**
 * POST /api/admin/distributors/orders/[orderId]/approve
 * Admin approves distributor order and creates Stripe invoice
 * WITH TRANSACTION SUPPORT: Rolls back Stripe invoice if database operations fail
 */
...
export async function POST(
  ...
  try {
    ...
    const stripeInvoice = await stripe.invoices.create({
      ...
      days_until_due: 30,
      auto_advance: false,
      description: `Distributor Order ${orderId}${order.po_number ? ` (PO: ${order.po_number})` : ''}`,
      metadata: {
        order_id: orderId,
        company_id: order.company_id,
        source: 'distributor_order',
