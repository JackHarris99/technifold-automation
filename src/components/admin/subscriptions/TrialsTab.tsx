/**
 * Trial Offers Tab
 * Shows trial intents (offer links sent to prospects)
 */
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-gray-800">Loading trial offers...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <h3 className="text-sm font-semibold text-blue-900 mb-2">Trial Offer Links</h3>
        <p className="text-sm text-blue-800">
          Track trial offer links sent to prospects. Active trials are valid for 7 days.
          Once converted to a subscription, they appear in the Active Subscriptions tab.
        </p>
      </div>
      ...
      {filteredTrials.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">
            No {filter !== 'all' ? filter : ''} trial offers found
          </h3>
          <p className="text-gray-700">
            Trial offers are created when prospects request a trial from machine pages
          </p>
        </div>
      ) : (
              ...
              {filteredTrials.map((trial) => (
                <tr key={trial.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-700">{trial.contact_email || '—'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{trial.company_name || '—'}</div>
                    {trial.company_id && (
                      <Link href={`/admin/company/${trial.company_id}`} className="text-xs text-blue-600 hover:underline">
                        View company
                      </Link>
                    )}
                  </td>
        ...
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <dt className="text-sm font-medium text-gray-700 truncate">Total Trial Offers</dt>
