...
export default function EditDeliveryAddressModal({
    ...
    async function loadAddress() {
      ...
      try {
        ...
        if (data.success || data.addresses) {
          ...
          const address = addresses.find((addr: any) => addr.address_id === addressId);
        ...
        console.error('[EditDeliveryAddressModal] Failed to load address:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = useDistributorEndpoints
        ? `/api/distributor/shipping-addresses/${addressId}`
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to update delivery address');
      }
      ...
      console.error('[EditDeliveryAddressModal] Error:', err);
    ...
    >
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 relative">
          ...
          <h2 className="text-2xl font-semibold">Edit Delivery Address</h2>
          <p className="text-sm text-gray-600 mt-2">
            Update an existing shipping/delivery address
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {isLoadingAddress && (
            <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4 text-center">
              ...
              <span className="text-sm font-semibold text-blue-900">Loading address...</span>
            </div>
          )}
          ...
          {isSubmitting && (
            <div className="bg-green-50 border-2 border-green-400 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-green-900">Updating delivery address...</p>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button
              ...
              disabled={isLoadingAddress || isSubmitting}
            >
              {isLoadingAddress ? 'Loading...' : isSubmitting ? 'Saving...' : 'Update Delivery Address'}
            </button>
          </div>
        </form>
      </div>
