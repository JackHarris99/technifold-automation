/**
 * Fix sync_invoice_to_product_history trigger to skip NULL product_codes
 * This was causing invoice status updates to fail silently when line items had NULL product_code
 */
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.payment_status = 'paid') OR
    ...
    FOR v_item IN
      ...
      FROM invoice_items
      WHERE invoice_id = NEW.invoice_id
      AND product_code IS NOT NULL  -- CRITICAL FIX: Skip NULL product_codes
    LOOP
      -- Get product type
      ...
      -- Only insert if we found a valid product type
